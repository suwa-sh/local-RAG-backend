...
class GraphitiLLMConfig(BaseModel):
    ...
    def create_client(self) -> LLMClient:
        ...
        # OpenAI API互換エンドポイントのbase_url対応
        if self.base_url:
            # base_urlが指定されている場合は、カスタムOpenAIClientを作成
            from openai import AsyncOpenAI

            custom_client = AsyncOpenAI(api_key=self.api_key, base_url=self.base_url)
            return OpenAIClient(config=llm_client_config, client=custom_client)
        else:
...
class MCPConfig(BaseModel):
    ...
    def from_cli(cls, args: argparse.Namespace) -> "MCPConfig":
        ...
        # Transport は SSE 固定（運用シンプル化のため）
        return cls(transport="sse")
...
async def initialize_server() -> MCPConfig:
    ...
    parser.add_argument(
        ...
        "If not provided, defaults to GROUP_ID environment variable, or a random UUID will be generated.",
    )
    # Transport は SSE 固定とする（運用シンプル化のため）
    # parser.add_argument(
    #     '--transport',
    #     choices=['sse', 'stdio'],
    #     default='sse',
