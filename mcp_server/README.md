...
- `--group-id`: Set a namespace for the graph (optional). If not provided, defaults to GROUP_ID environment variable, or "default".
- `--transport`: **REMOVED** - Transport is fixed to SSE for simplified operation.
...
```bash
python graphiti_mcp_server.py --use-custom-entities --group-id <your_group_id>
```

Hint: specify a `group_id` to namespace graph data, or set GROUP_ID environment variable. If neither is provided, the server will use "default" as the group_id.
