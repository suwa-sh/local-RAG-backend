...
class GraphitiService:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # local-RAG-backend: rerank(cross_encoder)を OpenAI互換エンドポイント +
            # RERANK_MODEL_NAME で構成する。
            # upstream / graphiti-core は cross_encoder 未指定だと既定の
            # OpenAIRerankerClient(api.openai.com / gpt-4.1-nano)を生成し、
            # LLM_MODEL_URL も RERANK_MODEL_NAME も無視してしまう。
            # search_for_rag は cross_encoder reranker を使うため、ここで明示的に
            # LLM と同じエンドポイント・キーを使うよう配線する。
            cross_encoder = None
            openai_provider = getattr(self.config.llm.providers, 'openai', None)
            if openai_provider and openai_provider.api_key:
                ...
                rerank_model = os.getenv('RERANK_MODEL_NAME') or None
                cross_encoder = OpenAIRerankerClient(
                    config=CoreLLMConfig(
                        api_key=openai_provider.api_key,
                        base_url=openai_provider.api_url,
                        model=rerank_model,  # None の場合 reranker 既定モデルを使用
                    )
                )
...
async def search_for_rag(
    ...
    """Search graph memory for RAG and return a ready-to-use context string.

    Performs an integrated hybrid search (facts, entities, episodes, communities)
    and returns the results formatted as a single context string suitable for
    feeding into an LLM prompt.
    ...
    try:
        ...
        # search_ の既定 config は cross_encoder reranker を使う。
        # GraphitiService.initialize() で cross_encoder を配線済みのため、
        # LLM_MODEL_URL / RERANK_MODEL_NAME が反映される。
        ...
        error_msg = str(e)
        logger.error(f'Error searching for rag: {error_msg}')
        return ErrorResponse(error=f'Error searching for rag: {error_msg}')
